...
def faculty_at_institution(institution_name, asst_faculty):
    ...
    for person in asst_faculty:
        if person.place == institution_name:
            current_faculty.append(person)
    return current_faculty

# Plot data with line of best fit
def plot_pubs_versus_prestige(data, ylabel, function=np.average, ylim=(0, 200)):
    pubs_by_prestige = {}
    ...
    df = pd.DataFrame(np.array(x), columns=["pubs", "prestige"])
    lm = smf.ols(formula='pubs ~ prestige ', data=df).fit()
    print(lm.summary())
    pickle.dump(df, open("real_data.p", "wb"))
    pickle.dump(lm, open("linear_regression.p", 'wb'))
    ...
    private_fit = r"%.4f" % (regr_private.coef_[0])
    public_fit = r"%.4f" % (regr_public.coef_[0])
    ...
    plt.legend(
        (
            ...
            private_fit,
            public_fit
        ), numpoints=1, loc='upper center', frameon=False, fontsize=LABEL_SIZE, ncol=1)
